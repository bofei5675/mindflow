...
2. 读取 `Domain-Map/_index.md`（索引页）找到相关 domain，再读取对应的 `Domain-Map/{Name}.md` 了解当前认知状态。
...
- **不直接修改 Domain-Map**：综合分析中如有值得纳入 Domain-Map 的发现，在 Survey 文件的 Key Takeaways 中标注"建议加入 Domain-Map"，不得直接修改 `Domain-Map/` 下的任何文件。
