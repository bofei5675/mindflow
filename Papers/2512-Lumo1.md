...
## Connections
- Related papers:
  - [[2512-Motus]] - 同期 VLA 工作，同样采用三阶段训练和 cross-embodiment 策略，但 Motus 侧重 world model 而 Lumo-1 侧重 embodied reasoning
  - π₀ / π₀.5 - 主要对比 baseline，generalist robot policy
  - RDT-1B, GR3 - 同类 generalist manipulation policy
  - RoboBrain, Robix - embodied VLM baseline
- Related ideas:
  - Chain-of-thought reasoning 在 robotics 中的应用——Lumo-1 将 CoT 推广为包含 visual grounding 的 structured reasoning
  - Flow-matching 作为 action head 的趋势（dual representation：discrete pre-train + continuous fine-tune 是有意思的设计选择）
- Related projects:
  - Astribot S1 双臂移动操作平台
